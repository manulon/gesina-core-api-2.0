...
def get_activity():
    labels, success_count, error_count = activity_service.get_activity_data()
    date_from = request.args.get('date_from', None)
    date_to = request.args.get('date_to', None)
    data = []
    for i in range(len(labels)):
        total_executions = success_count[i] + error_count[i]
        data.append({labels[i] : "No activity" if total_executions == 0 else success_count[i] / total_executions})
    return jsonify(data)
